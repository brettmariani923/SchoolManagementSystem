...
namespace Teachers.Api.Controllers
{
    ...
    public sealed class StudentsController : ControllerBase
    {
        ...
        // GET: api/students
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Students_DTO>>> GetAll()
        {
        ...
        // GET: api/students/5
        [HttpGet("{id:int}")]
        public async Task<ActionResult<Students_DTO>> GetById(int id)
        {
        ...
        // POST: api/students
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] StudentRequest request)
        {
            if (request is null) return BadRequest("Body required.");

            var rows = await _students.InsertAsync(request);
        ...
        [HttpPost("bulk")]
        public async Task<ActionResult> BulkInsert([FromBody] IEnumerable<StudentRequest> requests)
        {
            if (requests is null) return BadRequest("Body required.");

            var rows = await _students.InsertBulkAsync(requests);
        ...
        // PUT: api/students/5
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] StudentRequest body)
        {
            ...
            var rows = await _students.UpdateAsync(dto);
            return rows == 0 ? NotFound() : NoContent();
        }

        // PUT: api/students/bulk
        [HttpPut("bulk")]
        public async Task<IActionResult> BulkUpdate([FromBody] IEnumerable<Students_DTO> dtos)
        {
            ...
            var rows = await _students.UpdateBulkAsync(dtos);
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            var rows = await _students.RemoveByIdAsync(id);
        ...
        public async Task<ActionResult> BulkDelete([FromQuery] IEnumerable<int> ids)
        {
            ...
            var rows = await _students.RemoveBulkAsync(ids);
